...
$result = /*REMOVED SQL CODE | https://i.imgur.com/qBLCOHW.png*/

foreach($result as $row){
    $identifier_streamid = $row['redirect'];
    $identifier_season_title = $row['title'];
...
$json_episodes           = json_decode(get_episodes("kawa", $identifier_streamid), true);
$episode_count           = 1;
$generated_episodes_html = "";

$episode_list = array();

foreach ($json_episodes as $episode_number) {

}

foreach($json_episodes as $episode){
    if($episode == $identifier_episode){
        $paramurl = "v3.php?name=$identifier_slugtitle_full&ep=$episode_count";
...
$json = json_decode(get_streams("kawa", array("id" => $identifier_streamid, "ep" => $identifier_episode_current)), true);

foreach($json as $episode){
    $stream = $episode[0]['file'];
    $makeurl = $stream;
}

//echo $identifier_season_title;
$simplied_title = strlen($identifier_slugtitle_full) > 95 ? substr($identifier_slugtitle_full,0,95)."..." : $identifier_slugtitle_full;
$mal   = fetch_search_anime(smallifyTitle($simplied_title));
//echo '<script>alert("' . smallifyTitle($simplied_title) . '");</script>';
...
<html lang="en">
  ...
  <body>
      ...
      <div class="subpart eptitle">
        <div id="eptitle">
          <span id="eptitleplace">
            ...
            echo "EP" . $identifier_episode_current; 
            ?>
          </span>
        ...
        <div id="toprightplayer">
          ...
          $paramurl = "v3.php?name=$identifier_slugtitle_full&ep=$total";
          echo $paramurl;
	...
	var season_title = "<?php echo preg_replace('/\s+/', '', ltrim($identifier_season_title)); ?>"
	var episode_current = "<?php echo $identifier_episode_current; ?>"
    </script>
	<?php include 'players/v3_player.php'; ?>
  </body>
</html>